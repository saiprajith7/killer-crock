...
Current package: **boss-sentinel 1.2.10**
...
```bash
cd ~/Downloads
curl -L -o boss-sentinel_1.2.10-1_all.deb \
  https://raw.githubusercontent.com/saiprajith7/killer-crock/cursor/fix-invalid-class-name-02f6/releases/boss-sentinel_1.2.10-1_all.deb
curl -L -o install.sh \
...
```bash
sudo apt-get install -y ./boss-sentinel_1.2.10-1_all.deb
```

## ISO seed packages

Include `boss-sentinel_1.2.10-1_all.deb` plus:
